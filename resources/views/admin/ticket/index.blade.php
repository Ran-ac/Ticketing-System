            ...
            <main>
                <div class="container">
                    ...
                    <ol class="breadcrumb mb-2">
                        <li class="breadcrumb-item active">This is where you can add ticket</li>
                    </ol>

                    <div class="d-flex justify-content-between mb-3">
                        <button class="btn btn-success" id="openModalCreate">Create New Ticket</button>
                    </div>
                    <div class="card mb-4">
                        <div class="card-body">
...
$(document).ready(function () {
        ...
        let table = $("#ticketTable").DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.ticket.fetchTicketData') }}",
            columns: [
                { data: "id", name: "id" },
                    ...
                    render: function (data, type, row) {
                        return `
                            <div class="d-flex m-2">
                                ...
                                <button class="btn btn-danger deleteTicket" data-id="${data}">Delete</button>
                            </div>
                        `;
                    },
                    orderable: false
        ...
        $('#openModalCreate').click(function () {
            $.get("{{ route('admin.ticket.create') }}", function (data) {
        ...
        $(document).on('click', '.openEditModal', function (event) {
			...
			let ticketId = $(this).data('id');
	   		...
	   		$.get("{{ route('admin.ticket.edit', ':id') }}".replace(':id', ticketId), function (data) {
        ...
        $(document).on("click", ".deleteTicket", function (event) {
			...
			$.ajax({
				type: "DELETE",
				url: "/admin/ticket/delete-ticket/" + userId,
				data: {
					_token: "{{ csrf_token() }}"
				},
